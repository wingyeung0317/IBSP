...
// Example: "http://your-server.com/api/sensor-data"
...
const unsigned long REALTIME_TX_INTERVAL = 300000;  // Send realtime data every 5 minutes
...
void loop() {
  ...
  if (wifiComm.isJoined()) {
    ...
    if (fall_event.confirmed && !fallEventTriggered) {
      ...
      if (wifiComm.sendUplink(3, payload, len, true)) {  // Type 3, fall event
    ...
    // Send real-time monitoring data every 5 minutes (Packet Type 0x01)
    if (currentTime - lastRealtimeTxTime >= REALTIME_TX_INTERVAL) {
      Serial.println("\n>>> Sending Real-time Monitoring via WiFi <<<");
      ...
      if (wifiComm.sendUplink(1, payload, len, false)) {  // Type 1, realtime
        Serial.println("Real-time data sent successfully!");
        lastRealtimeTxTime = currentTime;
      }
    }
    
    // Send ECG data periodically (Packet Type 0x02) - every 30 seconds when heart rate stable
    static unsigned long lastECGTxTime = 0;
    if (currentTime - lastECGTxTime >= 30000) {
      int bpm = ecgMonitor.getBPM();
      if (bpm > 40 && bpm < 150) {  // Only send when heart rate in reasonable range
        Serial.println("\n>>> Sending ECG Data via WiFi <<<");
        ...
        if (wifiComm.sendUplink(2, payload, len, false)) {  // Type 2, ECG
